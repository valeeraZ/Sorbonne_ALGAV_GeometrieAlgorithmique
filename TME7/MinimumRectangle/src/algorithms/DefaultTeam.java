...
public class DefaultTeam {
  ...
  public Line calculDiametreOptimise(ArrayList<Point> points) {
    ...
    ArrayList<Line> antipodals = pairAntipodals(points);

    Point p = antipodals.get(0).getP();
    Point q = antipodals.get(0).getQ();

    for (Line a: antipodals)
  ...
  private ArrayList<Line> pairAntipodals(ArrayList<Point> points) {
    ...
    while (i<=k && j<n) {
      while (distance(p.get(j),p.get(i),p.get(i+1))<distance(p.get((j+1)%n),p.get(i),p.get(i+1)) && j<n-1) {
        antipodals.add(new Line(p.get(i),p.get(j)));
        j++;
      }
      antipodals.add(new Line(p.get(i),p.get(j)));
      i++;
